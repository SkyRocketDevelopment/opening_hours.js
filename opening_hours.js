...
var index = function(value, nominatim_object, optional_conf_parm) {
  ...
  var word_value_replacement = {
    // If the correct values can not be calculated.
    dawn: 60 * 5 + 30,
    sunrise: 60 * 6,
    sunset: 60 * 18,
    dusk: 60 * 18 + 30
  ...
  var osm_tag_defaults = {
    ...
    lit: {
      ...
      map: {
        yes:
          'sunset-sunrise open "specified as yes: At night (unknown time schedule or daylight detection)"',
        automatic:
          'unknown "specified as automatic: When someone enters the way the lights are turned on."',
  ...
  var t = function(str, variables) {
    if (
      ...
      var global_locale = i18n.language();
  ...
  if (typeof nominatim_object === "object" && nominatim_object !== null) {
    ...
    if (
      typeof nominatim_object.lon === "string" &&
      typeof nominatim_object.lat === "string"
    ) {
    ...
    do {
      ...
      new_tokens.push([
        tokens[nrule][0].slice(
          ...
          continue_at === 0 ? tokens[nrule][0].length : continue_at
        ),
        tokens[nrule][1],
        tokens[nrule][2]
      ]);
      ...
      if (next_rule_is_additional && new_tokens.length > 1) {
        ...
        new_tokens[new_tokens.length - 1][0].unshift(
          new_tokens[new_tokens.length - 2][0].pop()
        );
      }

      next_rule_is_additional = continue_at === 0 ? false : true;

      var optimal_selector_order = [
        "year",
        "holiday",
        "month",
        "monthday",
  ...
  function formatWarnErrorMessage(nrule, at, message, tokens_to_use) {
    ...
    if (typeof nrule === "number") {
      var pos = 0;
      if (nrule === -1) {
        // Usage of rule index not required because we do have access to value.length.
        pos = value.length - at;
      } else {
        // Issue occurred at a later time, position in string needs to be reconstructed.
        if (typeof tokens_to_use[nrule][0][at] === "undefined") {
          if (typeof tokens_to_use[nrule][0] && at === -1) {
            pos = value.length;
            if (
              typeof tokens_to_use[nrule + 1] === "object" &&
              typeof tokens_to_use[nrule + 1][2] === "number"
            ) {
            ...
            if (typeof tokens_to_use[nrule][2] === "number") {
              ...
              pos -= tokens_to_use[nrule][2];
              console.warn(
                "Last token for rule: " + JSON.stringify(tokens_to_use[nrule])
              );
  ...
  function tokenize(value) {
    ...
    while (value !== "") {
      ...
      if (typeof token_from_map === "object") {
        ...
        var ok = "";
        if (
          curr_rule_tokens.length > 0 &&
          matchTokens(curr_rule_tokens, curr_rule_tokens.length - 1, "number")
        ) {
          ok += ":";
        }
        ok += regular_number;
         ...
         */
        var correct_val = returnCorrectWordOrToken(
          tmp[1].toLowerCase(),
          value.length
        );
        ...
        if (typeof correct_val === "object") {
          ...
          if (correct_val === "am" || correct_val === "pm") {
            ...
            if (hours_token_at >= 0) {
              if (
                ...
                matchTokens(curr_rule_tokens, hours_token_at, "number")
              ) {
                hours_token = curr_rule_tokens[hours_token_at];
              }
          ...
          for (var i = 0; i < correct_tokens[0].length; i++) {
            curr_rule_tokens.push([
              correct_tokens[0][i][0],
              correct_tokens[0][i][1],
              value.length
            ]);
        ...
        value = value.substr(
          tmp[1].length + (typeof tmp[2] === "string" ? tmp[2].length : 0)
        );
        if (
          typeof tmp[2] === "string" &&
          tmp[2] !== "" &&
  ...
  function getWarnings(it) {
    ...
    if (!done_with_warnings && typeof it === "object") {
      ...
      var small_range_selector_order = [
        "weekday",
        "time",
        "24/7",
        "state",
      ...
      for (var nrule = 0; nrule < new_tokens.length; nrule++) {
        ...
        do {
          selector_start_end_type = getSelectorRange(
            new_tokens[nrule][0],
          ...
          for (
            var token_pos = 0;
            token_pos <= selector_start_end_type[1];
            token_pos++
          ) {
            if (
              typeof new_tokens[nrule][0][token_pos] === "object" &&
              new_tokens[nrule][0][token_pos][0] === "PH"
            ) {
              has_token["PH"] = true;
            }
          }
          ...
          if (
            selector_start_end_type[0] === selector_start_end_type[1] &&
          ...
          if (
            typeof used_selectors[nrule][selector_start_end_type[2]] !==
            "object"
          ) {
            used_selectors[nrule][selector_start_end_type[2]] = [
              selector_start_end_type[1]
            ];
          } else {
            used_selectors[nrule][selector_start_end_type[2]].push(
              selector_start_end_type[1]
            );
          }
      ...
      for (var nrule = 0; nrule < used_selectors.length; nrule++) {
        ...
        Object.keys(used_selectors[nrule]).forEach(function(selector_type) {
          ...
          if (used_selectors[nrule][selector_type].length > 1) {
            parsing_warnings.push([
              nrule,
              used_selectors[nrule][selector_type][
                used_selectors[nrule][selector_type].length - 1
              ],
              t("use multi", {
                count: used_selectors[nrule][selector_type].length,
                part2: /^(?:comment|state)/.test(selector_type)
                  ? t("selector multi 2a", {
                      what:
                        selector_type === "state"
                          ? t("selector state")
                  ...
                  : t("selector multi 2b", {
                      what: t(
                        selector_type +
                          (/^(?:month|weekday)$/.test(selector_type)
                            ? "s"
                            : " ranges")
                      )
                    })
        ...
        if (
          ...
          if (
            (typeof used_selectors[nrule].state === "object" &&
              new_tokens[nrule][0][used_selectors[nrule].state[0]][0] ===
                "open" &&
        ...
        if (
          typeof used_selectors[nrule].comment === "object" &&
          new_tokens[nrule][0][used_selectors[nrule].comment[0]][0].length === 0
        ) {
          parsing_warnings.push([
            nrule,
            used_selectors[nrule].comment[0],
            t("empty comment")
          ]);
        }
        /* }}} */
        ...
        for (var i = 0; i < used_selectors_types_array[nrule].length - 1; i++) {
          ...
          if (
            (wide_range_selector_order.indexOf(selector_type) !== -1 &&
            ...
            if (
              new_tokens[nrule][0][
                used_selectors[nrule][selector_type][0]
              ][0] === ":"
            ) {
              parsing_warnings.push([
                nrule,
        ...
        if (
          ...
          var rules_too_complex = [nrule, nrule + 1].map(function(nrule) {
            for (var i = 0; i < wide_range_selector_order.length - 1; i++) {
              if (
                typeof used_selectors[nrule][wide_range_selector_order[i]] ===
                "object"
              ) {
                return true;
              }
          ...
          if (
            ...
            for (var i = 0; i < rule_infos[nrule]["week_days"].length; i++) {
              ...
              if (
                rule_infos[nrule + 1]["week_days"].indexOf(
        ...
        if (
          typeof new_tokens[nrule][0][0] === "object" &&
      ...
      if (
        used_selectors_types_array.length > 1 &&
    ...
    for (var i = 0; i < parsing_warnings.length; i++) {
      warnings.push(
        formatWarnErrorMessage(
          parsing_warnings[i][0],
          parsing_warnings[i][1],
          parsing_warnings[i][2],
          parsing_warnings[i][3]
        )
      );
    }
    return warnings;
  ...
  function getSelectorRange(tokens, at) {
    ...
    if (pos_in_token_array === 1) {
      ...
      if (
        selector_end + 1 < tokens.length &&
        tokens[selector_end + 1][0] === ":"
      )
        selector_end++;
      ...
      return [
        selector_start,
        selector_end,
        tokens[selector_start][pos_in_token_array]
      ];
    }
  ...
  function prettifyValue(argument_hash) {
    ...
    if (
      ...
      typeof user_conf["locale"] === "string" &&
      user_conf["locale"] !== "en"
    ) {
      // FIXME: Does not work?
    ...
    for (var nrule = 0; nrule < new_tokens.length; nrule++) {
      ...
      if (typeof rule_index === "number") {
        ...
        if (nrule !== 0)
          prettified_value += new_tokens[nrule][1]
      ...
      if (!done_with_selector_reordering) {
        prettified_group_value.sort(function(a, b) {
          ...
          return (
            selector_order.indexOf(a[0][2]) - selector_order.indexOf(b[0][2])
          );
        });
      }
        ...
        for (var i = 0; i < prettified_group_value.length; i++) {
          ...
          if (type === "weekday") {
            weekdays_en.forEach(function(weekday, key) {
              prettified_group_value[i][1] = prettified_group_value[
                i
              ][1].replace(new RegExp(weekday, "g"), weekdays_local[key]);
            });
          } else if (type === "month") {
            months_en.forEach(function(month, key) {
              prettified_group_value[i][1] = prettified_group_value[
                i
              ][1].replace(new RegExp(month, "g"), months_local[key]);
            });
          } else {
            prettified_group_value[i][1] = i18n.t([
              "opening_hours:pretty." + prettified_group_value[i][1],
              prettified_group_value[i][1]
            ]);
          }
        }
        if (global_locale !== locale) {
      ...
      prettified_value += prettified_group_value
        .map(function(array) {
          return array[1];
        })
        .join(" ");
      ...
      if (!done_with_selector_reordering_warnings) {
        for (
          var i = 0, l = not_sorted_prettified_group_value.length;
          i < l;
          i++
        ) {
          if (
            not_sorted_prettified_group_value[i] !== prettified_group_value[i]
          ) {
  ...
  function parseGroup(tokens, at, rule, nrule) {
    ...
    while (at < tokens.length) {
      ...
      if (matchTokens(tokens, at, "weekday")) {
        ...
        matchTokens(tokens, at, "month", "number") ||
        ...
        if (!done_with_warnings && matchTokens(tokens, at - 1, "holiday")) {
          parsing_warnings.push([
            nrule,
            at,
            t("no colon after", { token: tokens[at - 1][1] })
          ]);
        }

        at++;
      } else if (
        matchTokens(tokens, at, "number", "timesep") ||
        ...
        (at === 0 || at === tokens.length - 1) &&
        matchTokens(tokens, at, "rule separator")
      ) {
        at++;
        ...
        throw formatWarnErrorMessage(
          nrule,
          at,
          t("unexpected token", { token: tokens[at][1] })
  ...
  function dateAtNextWeekday(date, weekday) {
    ...
    return new Date(
      date.getFullYear(),
      date.getMonth(),
      date.getDate() + delta + (delta < 0 ? 7 : 0)
    );
  }
  /* }}} */
    ...
    if (period === 0) {
      ...
      if (typeof parm_string === "string" && parm_string === "no_end_year")
        parsing_warnings.push([
          nrule,
          at,
          t("period one year+", { type: period_type })
        ]);
      else
        parsing_warnings.push([
          nrule,
          at,
          t("period one", { type: period_type })
        ]);
    }
  }
   ...
   * :returns: Date object.
   */
  function getDateForConstrainedWeekday(
    year,
    month,
    weekday,
    constrained_weekday,
    ...
    var tmp_date = dateAtNextWeekday(
      new Date(year, month + (constrained_weekday[0] > 0 ? 0 : 1), 1),
      weekday
    );

    tmp_date.setDate(
      tmp_date.getDate() +
        (constrained_weekday[0] + (constrained_weekday[0] > 0 ? -1 : 0)) * 7
    );
  ...
  function checkIfDateIsValid(month, day, nrule, at) {
    ...
    if (day < 1 || day > 31) {
      throw formatWarnErrorMessage(
        nrule,
        at,
        t("month 31", { month: months$$1[month] })
      );
    } else if (
      ...
      throw formatWarnErrorMessage(
        nrule,
        at,
        t("month 30", { month: months$$1[month] })
      );
    } else if (month === 1 && day === 30) {
      throw formatWarnErrorMessage(
        nrule,
        at,
        t("month feb", { month: months$$1[month] })
      );
    }
  }
  /* }}} */
  ...
  function parseTimeRange(tokens, at, rule, extended_open_end, nrule) {
    ...
    for (; at < tokens.length; at++) {
      var has_time_var_calc = [],
        has_normal_time = []; // element 0: start time, 1: end time
      has_normal_time[0] = matchTokens(
        tokens,
        at,
        "number",
        "timesep",
      ...
      if (
        ...
        if (has_normal_time[0]) {
          minutes_from = getMinutesByHoursMinutes(
            tokens,
            nrule,
            at + has_time_var_calc[0]
          );
        } else {
        ...
        var at_end_time =
          at + (has_normal_time[0] ? 3 : has_time_var_calc[0] ? 7 : 1) + 1; // after '-'
          ...
          if (matchTokens(tokens, at_end_time - 1, "+")) {
            ...
            if (oh_mode === 0) {
              throw formatWarnErrorMessage(
                ...
                at +
                  (has_normal_time[0]
                    ? typeof tokens[at + 3] === "object"
                      ? 3
                      : 2
                    : has_time_var_calc[0]
                    ? 2
                    : typeof tokens[at + 1] === "object"
                    ? 1
                    : 0),
                t("point in time", {
        ...
        if (has_open_end) {
          ...
          has_time_var_calc[1] = matchTokens(
            tokens,
            at_end_time,
            "(",
            "timevar"
          ...
          if (
            !has_normal_time[1] &&
            !matchTokens(tokens, at_end_time, "timevar") &&
            !has_time_var_calc[1]
          ) {
            throw formatWarnErrorMessage(
              nrule,
              at_end_time - (typeof tokens[at_end_time] === "object" ? 0 : 1),
        ...
        if (matchTokens(tokens, at, "/", "number")) {
          if (matchTokens(tokens, at + 2, "timesep", "number")) {
            // /hours:minutes
            point_in_time_period = getMinutesByHoursMinutes(
              tokens,
              nrule,
              at + 1
            );
          ...
          parseTimeRange(
            ...
            at_end_time,
            rule,
            minutes_to < minutes_from ? 1 : true,
            nrule
          );
          at++;
        ...
        if (typeof lat === "string") {
          // lon will also be defined (see above)
          if (
            !has_normal_time[0] ||
            !(has_normal_time[1] || has_open_end || is_point_in_time)
          ) {
            week_stable = false;
          }
        } else {
          // we can not calculate exact times so we use the already applied constants (word_value_replacement).
          timevar_string = [];
        }
        ...
        if (minutes_to > minutes_in_day * 2) {
          throw formatWarnErrorMessage(
            nrule,
            at_end_time +
              (has_normal_time[1] ? 4 : has_time_var_calc[1] ? 7 : 1) -
              2,
            t("two midnights")
          );
        }
        ...
        if (minutes_from === 0 && minutes_to === minutes_in_day) {
          ...
          if (minutes_to > minutes_in_day) {
            // has_normal_time[1] must be true
            rule.time.push(
              (function(
                minutes_from,
                minutes_to,
                ...
                return function(date) {
                  ...
                  if (timevar_string[0]) {
                    var date_from = SunCalc.getTimes(date, lat, lon)[
                      timevar_string[0]
                    ];
                    minutes_from =
                      date_from.getHours() * 60 +
                      date_from.getMinutes() +
                      timevar_add[0];
                  }
                  if (timevar_string[1]) {
                    var date_to = SunCalc.getTimes(date, lat, lon)[
                      timevar_string[1]
                    ];
                    minutes_to =
                      date_to.getHours() * 60 +
                    ...
                    is_point_in_time &&
                    typeof point_in_time_period !== "number"
                  ) {
                    minutes_to = minutes_from + 1;
                  }
                  ...
                  if (typeof point_in_time_period === "number") {
                    if (ourminutes < minutes_from) {
                      ...
                      for (
                        var cur_min = minutes_from;
                        ourminutes + point_in_time_period >= cur_min;
                        cur_min += point_in_time_period
                      ) {
                    ...
                    if (ourminutes < minutes_from)
                      ...
                      return [
                        true,
                        dateAtDayMinutes(date, minutes_to),
                        has_open_end,
                        extended_open_end
                      ];
                  }
                      ...
                      if (ourminutes <= minutes_to) {
                        for (
                          ...
                          if (cur_min === ourminutes) {
                            return [
                              true,
                              dateAtDayMinutes(date, ourminutes + 1)
                            ];
                          } else if (ourminutes < cur_min) {
                            return [false, dateAtDayMinutes(date, cur_min)];
                          }
                ...
                return function(date) {
                  ...
                  if (typeof point_in_time_period === "number") {
                    ...
                    if (ourminutes < minutes_from)
                      ...
                      return [
                        false,
                        dateAtDayMinutes(date, minutes_from + minutes_in_day)
                      ];
                  }
                };
              })(
        ...
        // "Mo 09-18" (Please don’t use this) -> "Mo 09:00-18:00".
        ...
        if (minutes_to > minutes_in_day) {
          rule.time.push(
            (function(minutes_from, minutes_to) {
              return function(date) {
                var ourminutes = date.getHours() * 60 + date.getMinutes();
        ...
        if (matchTokens(tokens, at, "("))
          throw formatWarnErrorMessage(
            nrule,
            at,
            'Missing variable time (e.g. sunrise) after: "' +
              tokens[at][1] +
              '"'
          );
        if (matchTokens(tokens, at, "number", "timesep"))
          throw formatWarnErrorMessage(
            nrule,
            at + 1,
            'Missing minutes in time range after: "' + tokens[at + 1][1] + '"'
          );
        if (matchTokens(tokens, at, "number"))
          throw formatWarnErrorMessage(
            nrule,
            at + (typeof tokens[at + 1] === "object" ? 1 : 0),
            'Missing time separator in time range after: "' +
              tokens[at][1] +
              '"'
          );
        return [at];
      ...
      if (typeof tokens[at + 1] === "undefined" && !done_with_warnings) {
        parsing_warnings.push([
          nrule,
          at,
          t("value ends with token", { token: tokens[at][1] })
        ]);
      }
    }
    ...
    if (matchTokens(tokens, at + 2, "+") || matchTokens(tokens, at + 2, "-")) {
      if (matchTokens(tokens, at + 3, "number", "timesep", "number")) {
        if (matchTokens(tokens, at + 6, ")")) {
          ...
          var minutes =
            getMinutesByHoursMinutes(tokens, nrule, at + 3) * add_or_subtract;
          if (minutes === 0)
    ...
    for (; at < tokens.length; at++) {
      if (matchTokens(tokens, at, "weekday", "[")) {
        ...
        for (var nnumber = 0; nnumber < numbers.length; nnumber++) {
          rule.weekday.push(
            (function(weekday, number, add_days) {
              return function(date) {
                var date_num = getValueForDate(date, false); // Year not needed to distinguish.
                var start_of_this_month = new Date(
                  date.getFullYear(),
                  date.getMonth(),
                  1
                );
                var start_of_next_month = new Date(
                  date.getFullYear(),
                  date.getMonth() + 1,
                  1
                );
                ...
                target_day_this_month = getDateForConstrainedWeekday(
                  date.getFullYear(),
                  date.getMonth(),
                  weekday,
                  [number]
                ...
                var target_day_with_added_days_this_month = new Date(
                  target_day_this_month.getFullYear(),
                  target_day_this_month.getMonth(),
                  target_day_this_month.getDate() + add_days
                );

                // The target day with added days can be before this month
                if (
                  target_day_with_added_days_this_month.getTime() <
                  start_of_this_month.getTime()
                ) {
                  // but in this case, the target day without the days added needs to be in this month
                  if (
                    target_day_this_month.getTime() >=
                    start_of_this_month.getTime()
                  ) {
                    ...
                    target_day_with_added_days_this_month = dateAtNextWeekday(
                      new Date(
                        date.getFullYear(),
                        date.getMonth() + (number > 0 ? 0 : 1) + 1,
                        1
                      ),
                      weekday
                    );
                    target_day_this_month.setDate(
                      target_day_with_added_days_this_month.getDate() +
                        (number + (number > 0 ? -1 : 0)) * 7 +
                        add_days
                    );
                  } else {
                ...
                if (add_days > 0) {
                  ...
                  target_day_with_added_moved_days_this_month.setDate(
                    target_day_with_added_moved_days_this_month.getDate() +
                      (number + (number > 0 ? -1 : 0)) * 7 +
                      add_days
                  );
                  ...
                  if (
                    date_num ===
                    getValueForDate(
                      target_day_with_added_moved_days_this_month,
                      false
                    )
                  )
                  ...
                  if (
                    target_day_with_added_moved_days_this_month.getTime() >=
                    start_of_next_month.getTime()
                  ) {
                    if (
                      target_day_with_added_days_this_month.getTime() >=
                    ...
                    if (
                      target_day_with_added_days_this_month.getTime() <
                        start_of_next_month.getTime() &&
                      getValueForDate(
                        target_day_with_added_days_this_month,
                        false
                      ) === date_num
                    )
                      return [true, dateAtDayMinutes(date, minutes_in_day)];
        ...
        var weekday_list = Array.apply(
          0,
          Array(weekday_to - weekday_from + 1)
        ...
        if (weekday_to < weekday_from) {
          ...
          rule.weekday.push(
            (function(weekday_from, weekday_to, inside) {
              return function(date) {
        ...
        throw formatWarnErrorMessage(
          nrule,
          at,
          t("unexpected token weekday range", { token: tokens[at][1] })
        );
      }
  ...
  function getMoveDays(tokens, at, max_differ, name) {
    ...
    add_days[0] =
      matchTokens(tokens, at, "+") || (matchTokens(tokens, at, "-") ? -1 : 0);
  ...
  function parseHoliday(
    ...
    if (!in_holiday_selector) {
      ...
      else tokens[at][3] = "holiday"; // Could also be holiday but this is not important here.
    }
      ...
      if (matchTokens(tokens, at, "holiday")) {
        if (tokens[at][0] === "PH") {
          ...
          var selector = (function(applying_holidays, add_days) {
            return function(date) {
              var holidays = getApplyingHolidaysForYear(
                applying_holidays,
                date.getFullYear(),
                add_days
              );
              ...
              for (var i = 0; i < holidays.length; i++) {
                var next_holiday_date_num = getValueForDate(
                  holidays[i][0],
                  true
                );
                ...
                if (date_num < next_holiday_date_num) {
                  if (add_days[0] > 0) {
                    ...
                    var holidays_last_year = getApplyingHolidaysForYear(
                      applying_holidays,
                      date.getFullYear() - 1,
                      add_days
                    );
                    var last_holiday_last_year =
                      holidays_last_year[holidays_last_year.length - 1];
                    var last_holiday_last_year_num = getValueForDate(
                      last_holiday_last_year[0],
                      true
                    );
              ...
              if (add_days[0] < 0) {
                ...
                var holidays_next_year = getApplyingHolidaysForYear(
                  applying_holidays,
                  date.getFullYear() + 1,
                  add_days
                );
                var first_holidays_next_year = holidays_next_year[0];
                var first_holidays_next_year_num = getValueForDate(
                  first_holidays_next_year[0],
                  true
                );
            ...
            return function(date) {
              ...
              for (var i = 0; i < applying_holidays.length; i++) {
                ...
                for (var h = 0; h < holiday.length; h += 4) {
                  var holiday_to_plus = new Date(
                    date.getFullYear(),
                    holiday[2 + h] - 1,
                    holiday[3 + h] + 1
                  );
                  var holiday_from =
                    (holiday[0 + h] - 1) * 100 + holiday[1 + h];
                  ...
                  if (date_num < holiday_from) {
                    // date is before selected holiday

                    // check if we are in the holidays from the last year spanning into this year
                    var last_year_holiday = getSHForYear(
                      applying_holidays[applying_holidays.length - 1],
                      date.getFullYear() - 1,
                      false
                    );
                    if (typeof last_year_holiday === "object") {
                      var last_year_holiday_from =
                        (last_year_holiday[last_year_holiday.length - 4] - 1) *
                          100 +
                        last_year_holiday[last_year_holiday.length - 3]; // e.g. 1125
                      var last_year_holiday_to =
                      ...
                      if (
                        last_year_holiday_from > last_year_holiday_to &&
                        date_num <= last_year_holiday_to
                      ) {
                        return [
                          true,
                          new Date(
                      ...
                      /* School holidays for last year are not defined. */
                      return [
                        false,
                        new Date(
                          date.getFullYear(),
                    ...
                    holiday_from <= date_num &&
                    (date_num <= holiday_to || holiday_ends_next_year)
                  ) {
                    return [
                      true,
                      new Date(
                        date.getFullYear() + holiday_ends_next_year,
                        holiday[2 + h] - 1,
                        holiday[3 + h] + 1
                      ),
                    ...
                    // selected holiday end is equal to month and day
                    if (h + 4 < holiday.length) {
                      // next holiday is next date range of the same holidays
                      h += 4;
                      return [
                        false,
                        new Date(
        ...
        throw formatWarnErrorMessage(
          nrule,
          at,
          t("unexpected token holiday", { token: tokens[at][1] })
        );
      }
  ...
  function getValueForDate(date, include_year) {
    ...
    return (
      (include_year ? date.getFullYear() * 10000 : 0) +
      date.getMonth() * 100 +
      date.getDate()
    );
  }
  ...
  function getMatchingHoliday(type_of_holidays) {
    ...
    if (
      ...
      typeof holiday_definitions[location_cc][location_state][
        type_of_holidays
      ] === "object"
    ) {
      ...
      if (type_of_holidays === "PH") {
        ...
        var country_holiday_names = country_holidays.map(function(
          country_holiday
        ) {
       ...
       * so that is ok too.
       */
      var applying_holidays_for_country =
        holiday_definitions[location_cc][type_of_holidays];
  ...
  function getMovableEventsForYear(year) {
    ...
    var lastFebruarySunday =
      lastFebruaryDay.getDate() - lastFebruaryDay.getDay();
    /* }}} */
     ...
     */
    function getDateOfNextWeekdayRange(
      first_weekday,
      last_weekday,
      start_date
    ) {
      ...
      if (
        first_weekday <= start_date.getDay() &&
        start_date.getDay() <= last_weekday
      ) {
        return start_date;
      } else {
    ...
    return {
      ...
      nextWednesday16Nov: getDateOfWeekdayInDateRange(
        3,
        new Date(year, 10, 16)
      ),
      "nextMo-Fr17March": getDateOfNextWeekdayRange(
        1,
        5,
        new Date(year, 2, 17)
      ),
      "nextMo-Sa01May": getDateOfNextWeekdayRange(1, 6, new Date(year, 4, 1)),
      "nextMo-Sa07August": getDateOfNextWeekdayRange(
        1,
        6,
        new Date(year, 7, 7)
      ),
      "nextMo-Sa25December": getDateOfNextWeekdayRange(
        1,
        6,
        new Date(year, 11, 25)
      )
  ...
  function getApplyingHolidaysForYear(applying_holidays, year, add_days) {
    ...
    applying_holidays.forEach(function(holiday_item) {
      if ("fixed_date" in holiday_item) {
        ...
        next_holiday = new Date(
          selected_movableDay.getFullYear(),
        ...
        throw formatLibraryBugMessage(
          "Unexpected object: " + JSON.stringify(holiday_item, null, "    ")
        );
      }
  ...
  function parseYearRange(tokens, at) {
    ...
    for (; at < tokens.length; at++) {
      if (matchTokens(tokens, at, "year")) {
        ...
        if (is_range && tokens[at + 2][0] <= year_from) {
          ...
          if (tokens[at + 2][0] === year_from) {
            throw formatWarnErrorMessage(
              nrule,
              at,
              t("year range one year", { year: year_from })
            );
          } else {
        ...
        rule.year.push(
          (function(tokens, at, year_from, is_range, has_period, period) {
            return function(date) {
        ...
        throw formatWarnErrorMessage(
          nrule,
          at,
          t("unexpected token year range", { token: tokens[at][1] })
        );
      }
    ...
    for (; at < tokens.length; at++) {
      ...
      if (matchTokens(tokens, at, "number")) {
        ...
        if (!period && week_from === 1 && week_to === 53) {
          ...
          rule.week.push(
            (function(week_from, week_to, period) {
              return function(date) {
                ...
                if (period) {
                  ...
                  if (in_period) {
                    ...
                    return [
                      false,
                      getNextDateOfISOWeek(ourweek + period - 1, date)
                    ];
                  }
                }
                ...
                return [
                  true,
                  getNextDateOfISOWeek(week_to === 53 ? 1 : week_to + 1, date)
                ];
              };
        ...
        throw formatWarnErrorMessage(
          nrule,
          at,
          t("unexpected token week range", { token: tokens[at][1] })
        );
      }
  ...
  function parseMonthRange(tokens, at, push_to_monthday, in_selector) {
    ...
    for (; at < tokens.length; at++) {
      ...
      if (
        matchTokens(tokens, at, "month", "number") &&
        !matchTokens(tokens, at + 2, "timesep", "number")
      ) {
        ...
        throw formatWarnErrorMessage(
          nrule,
          at,
          t("unexpected token month range", { token: tokens[at][1] })
        );
      }
  ...
  function dateAtNextMonth(date, month) {
    return new Date(
      date.getFullYear(),
      month < date.getMonth() ? month + 12 : month
    );
  }
  /* }}} */
  ...
  function parseMonthdayRange(tokens, at, nrule, push_to_month) {
    ...
    for (; at < tokens.length; at++) {
      ...
      if (matchTokens(tokens, at + has_year[0], "month", "weekday", "[")) {
        has_constrained_weekday[0] = getConstrainedWeekday(
          tokens,
          at + has_year[0] + 3
        );
        has_calc[0] = getMoveDays(
          tokens,
          has_constrained_weekday[0][1],
          6,
          "constrained weekdays"
        );
        at_range_sep =
          has_constrained_weekday[0][1] +
          (typeof has_calc[0] === "object" && has_calc[0][1] ? 3 : 0);
      } else {
        at_range_sep =
          at +
          has_year[0] +
          (has_event[0]
            ? typeof has_calc[0] === "object" && has_calc[0][1]
              ? 4
              : 1
            : 2);
      }
      ...
      if (
        (has_month[0] || has_event[0] || has_constrained_weekday[0]) &&
        matchTokens(tokens, at_range_sep, "-")
      ) {
        has_year[1] = matchTokens(tokens, at_range_sep + 1, "year");
        at_sec_event_or_month = at_range_sep + 1 + has_year[1];
        has_month[1] = matchTokens(
          tokens,
          at_sec_event_or_month,
          "month",
          "number"
        );
        if (!has_month[1]) {
          ...
          if (has_event[1]) {
            has_calc[1] = getMoveDays(
              tokens,
              at_sec_event_or_month + 1,
              366,
              "event like easter"
            );
          } else if (
            matchTokens(tokens, at_sec_event_or_month, "month", "weekday", "[")
          ) {
            has_constrained_weekday[1] = getConstrainedWeekday(
              tokens,
              at_sec_event_or_month + 3
            );
            has_calc[1] = getMoveDays(
              tokens,
      ...
      if (
        has_year[0] === has_year[1] &&
        (has_month[1] || has_event[1] || has_constrained_weekday[1])
      ) {
        if (has_month[0])
          checkIfDateIsValid(
            tokens[at + has_year[0]][0],
            tokens[at + has_year[0] + 1][0],
            nrule,
            at + has_year[0] + 1
          );
        if (has_month[1])
          checkIfDateIsValid(
            tokens[at_sec_event_or_month][0],
        ...
        var selector = (function(
          ...
          return function(date) {
            ...
            if (has_event[0]) {
              movableDays = getMovableEventsForYear(
                has_year[0] ? parseInt(tokens[at][0]) : date.getFullYear()
              );
              ...
              if (typeof has_calc[0] === "object" && has_calc[0][1]) {
                ...
                if (from_year_before_calc !== from_date.getFullYear())
                  throw formatWarnErrorMessage(
                    nrule,
                    at + has_year[0] + has_calc[0][1] * 3,
                    t("movable not in year", {
              ...
              from_date = getDateForConstrainedWeekday(
                has_year[0] ? tokens[at][0] : date.getFullYear(), // year
            ...
            if (has_event[1]) {
              ...
              if (typeof has_calc[1] === "object" && has_calc[1][1]) {
                ...
                if (to_year_before_calc !== to_date.getFullYear()) {
                  throw formatWarnErrorMessage(
                    nrule,
                    at_sec_event_or_month + has_calc[1][1],
                    t("movable not in year", {
              ...
              to_date = new Date(
                has_year[1]
                  ...
                  : date.getFullYear(),
                tokens[at_sec_event_or_month][0],
                tokens[at_sec_event_or_month + 1][0] + 1
              );
            }
        ...
        do {
          ...
          if (
            is_range &&
            matchTokens(tokens, at + has_year + 4, "/", "number")
          ) {
          ...
          checkIfDateIsValid(
            month,
            range_to - 1 /* added previously */,
            nrule,
            at + has_year + (is_range ? 3 : 1)
          );
          /* }}} */
          ...
          var selector = (function(
            ...
            return function(date) {
              ...
              var from_date = new Date(
                has_year ? year : date.getFullYear(),
                month,
                range_from
              );
              if (month === 1 && range_from !== from_date.getDate())
                // Only on leap years does this day exist.
              ...
              var nday = Math.floor(
                (date.getTime() - from_date.getTime()) / msec_in_day
              );
        ...
        var selector = (function(tokens, at, nrule, has_year, add_days) {
          return function(date) {
            ...
            var movableDays = getMovableEventsForYear(
              has_year ? tokens[at][0] : date.getFullYear()
            );
            ...
            // else if (date.getTime() < event_date.getTime() + msec_in_day) // does not work because of daylight saving times
              ...
              event_date.getMonth() * 100 + event_date.getDate() ===
        ...
        at +=
          has_year[0] +
          has_event[0] +
          (typeof has_calc[0][1] === "number" && has_calc[0][1] ? 3 : 0);
        /* }}} */
      } else if (has_constrained_weekday[0]) {
        at = parseMonthRange(tokens, at);
      } else if (matchTokens(tokens, at, "month")) {
  ...
  this.getStatePair = function(date) {
    ...
    for (var nrule = 0; nrule < rules.length; nrule++) {
      ...
      for (
        var ndateselector = 0;
        ndateselector < rules[nrule].date.length;
        ndateselector++
      ) {
        ...
        for (var datesel = 0; datesel < dateselectors.length; datesel++) {
          ...
          if (
            typeof changedate === "undefined" ||
            (typeof res[1] === "object" &&
              res[1].getTime() < changedate.getTime())
          )
            changedate = res[1];
        }
      ...
      if (matching_date_rule) {
        ...
        if (
          (rules[nrule].date.length > 0 ||
            (nrule > 0 &&
              rules[nrule].meaning &&
              rules[nrule - 1].date.length === 0)) &&
          (rules[nrule].meaning || rules[nrule].unknown) &&
          !rules[nrule].wrapped &&
          !rules[nrule].additional &&
          !rules[nrule].fallback
        ) {
    ...
    for (var nrule = 0; nrule < date_matching_rules.length; nrule++) {
      ...
      if (rules[rule].time.length === 0) {
        ...
        if (
          !rules[rule].fallback ||
          (rules[rule].fallback && !(resultstate || unknown))
        ) {
      ...
      for (var timesel = 0; timesel < rules[rule].time.length; timesel++) {
          ...
          if (
            ...
            if (res[2] === true && (resultstate || unknown)) {
              ...
              if (
                ...
                rules[rule].build_from_token_rule.toString() ===
                  rules[rule - 1].build_from_token_rule.toString() &&
                typeof rules[rule] === "object" &&
                rules[rule].build_from_token_rule.toString() ===
                  rules[rule].build_from_token_rule.toString()
              ) {
                var last_wrapping_time_selector =
                  rules[rule].time[rules[rule].time.length - 1];
                var last_w_res = last_wrapping_time_selector(
                ...
                if (
                  ...
                  (typeof last_w_res[2] === "undefined" ||
                    last_w_res[2] === false) && // Do not match for 'Tu 23:59-40:00+'
            ...
            if (rules[rule].fallback) {
              if (
                typeof changedate === "undefined" ||
                (typeof res[1] !== "undefined" && res[1] < changedate)
              ) {
        ...
        if (
          typeof changedate === "undefined" ||
          (typeof res[1] === "object" && res[1] < changedate)
        )
          changedate = res[1];
      }
    }
    ...
    if (
      typeof rules[match_rule] === "object" &&
      typeof rules[match_rule].comment === "string"
    ) {
  ...
  function prettifySelector(
    ...
    while (at <= selector_end) {
      ...
      if (matchTokens(tokens, at, "weekday")) {
        if (
          !conf.leave_weekday_sep_one_day_betw &&
          at - selector_start > 1 &&
          ...
          prettified_value =
            prettified_value.substring(0, prettified_value.length - 1) +
            conf.sep_one_day_between;
        }
        prettified_value += weekdays$$1[tokens[at][0]];
      } else if (
        at - selector_start > 0 && // e.g. '09:0' -> '09:00'
        selector_type === "time" &&
        ...
        selector_type === "time" && // e.g. '9:00' -> ' 09:00'
        conf.zero_pad_hour &&
        at !== tokens.length &&
        matchTokens(tokens, at, "number") &&
        matchTokens(tokens, at + 1, "timesep")
        ...
        selector_type === "time" && // e.g. '9-18' -> '09:00-18:00'
        at + 2 <= selector_end &&
        matchTokens(tokens, at, "number") &&
        matchTokens(tokens, at + 1, "-") &&
        ...
        prettified_value +=
          ":00-" +
          (tokens[at + 2][0] < 10 ? "0" : "") +
          tokens[at + 2][0].toString() +
          ":00";
        at += 2;
      } else if (matchTokens(tokens, at, "comment")) {
        prettified_value += '"' + tokens[at][0].toString() + '"';
      } else if (matchTokens(tokens, at, "closed")) {
        prettified_value += conf.leave_off_closed
          ? tokens[at][0]
          : conf.keyword_for_off_closed;
      } else if (
        at - selector_start > 0 &&
        matchTokens(tokens, at, "number") &&
        ...
        prettified_value +=
          " " +
          (conf.zero_pad_month_and_week_numbers && tokens[at][0] < 10
            ? "0"
            : "") +
          tokens[at][0];
      } else if (
        at - selector_start > 0 &&
        matchTokens(tokens, at, "month") &&
